...
struct LoganResultCUDA {
    int  score;
    bool    rc;
    LSeed seed;
};

struct LoganAlignmentInfoCUDA
{
...
// __inline__ __device__ void updateExtendedSeedL(LSeed& seed, ExtensionDirectionL direction, int &cols, int &rows, int &lowerDiag, int &upperDiag);
...
// __global__ void extendSeedLGappedXDropOneDirection( LSeed *seed, char *querySegArray, char *databaseSegArray, ExtensionDirectionL direction, int scoreDropOff, int *res, int *qL, int *dbL, int *offsetQuery, int *offsetTarget);
// inline void extendSeedL(vector<LSeed> &seeds, ExtensionDirectionL direction, vector<string> &target, vector<string> &query, vector<ScoringSchemeL> &penalties, int const& XDrop, int const& kmer_length);

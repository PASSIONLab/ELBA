...
// void apply_batch(DistributedFastaData& dfd, std::vector<CT<ReadOverlap>::ref_tuples>& mattuples, std::vector<uint64_t>& lids, std::vector<size_t>& ContainedSeqBatch, int mat, int mis, int gap, int dropoff)
// {
    // size_t npairs = mattuples.size();
    // std::vector<XSeed> ai(npairs);
    ...
    // {
        ...
        // {
            ...
            // int begQ = (cnt == 0)? std::get<0>(cks->begQs) : std::get<1>(cks->begQs);
            // int begT = (cnt == 0)? std::get<0>(cks->begTs) : std::get<1>(cks->begTs);

            // XSeed result;

            // const DnaSeq& seqQ = (*rowbuf)[std::get<0>(mattuples[lids[i]])];
            // const DnaSeq& seqT = (*colbuf)[std::get<1>(mattuples[lids[i]])];

            // int score = _xdrop_aligner(seqQ, seqT, begQ, begT, mat, mis, gap, dropoff, result);

            // bool rc = result.rc;
            // int lenQ = seqQ.size();
            // int lenT = seqT.size();
            // uint64_t idxQ = std::get<0>(mattuples[lids[i]]) + rowoffset;
            // uint64_t idxT = std::get<1>(mattuples[lids[i]]) + coloffset;
            // int begQr = ai[i].begQ;
            // int endQr = ai[i].endQ;
    ...
    // std::vector<size_t> ContainedSeqs;

    // for (size_t i = 0; i < npairs; ++i)
    // {
        // OverlapClass kind;

        // int lenQ = (*rowbuf)[std::get<0>(mattuples[lids[i]])].size();
        // int lenT = (*colbuf)[std::get<1>(mattuples[lids[i]])].size();
        ...
        // std::get<0>(cks->b) = ai[i].begQ;
        // std::get<1>(cks->b) = ai[i].endQ;

        // std::get<0>(cks->e) = ai[i].begT;
        // std::get<1>(cks->e) = ai[i].endT;

        // std::get<0>(cks->l) = lenQ;
        // std::get<1>(cks->l) = lenT;
        ...
        // {
            ...
            // {
                // ContainedSeqs.push_back(idxQ);
            // }
            // else if (kind == SECOND_CONTAINED)
            // {
                // ContainedSeqs.push_back(idxT);
            // }
    ...
    // int readcount = 0;
    // readcount += ContainedSeqs.size();

    // ContainedSeqBatch.resize(readcount);

    // std::copy(ContainedSeqs.begin(), ContainedSeqs.end(), ContainedSeqBatch.begin());
// }

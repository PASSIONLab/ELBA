...
template <int NLONGS>
Kmer<NLONGS> Kmer<NLONGS>::GetExtension(char const nt) const
{
    Kmer ext;
    ...
    ext.longs[NLONGS-1] |= (static_cast<uint64_t>(DnaSeq::getcharcode(nt)) << (2 * (32 - (KMER_SIZE%32))));
...
template <int NLONGS>
std::vector<Kmer<NLONGS>> Kmer<NLONGS>::GetKmers(const std::string& s)
{
    int l = s.size();
...
template <int NLONGS>
std::vector<Kmer<NLONGS>> Kmer<NLONGS>::GetRepKmers(const std::string& s)
{

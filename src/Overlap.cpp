...
void Overlap::extend_overlap(const DnaSeq& seqQ, const DnaSeq& seqT, int mat, int mis, int gap, int min_overlap_len, double target_identity, int dropoff)
{
    ...
    OverlapClass kind;
    classify_alignment(result, seqQ.size(), seqT.size(), min_overlap_len, target_identity, kind);
